...
```bash
python-app-dev$ sam build --use-container
```
...
```bash
python-app-dev$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
python-app-dev$ sam local start-api
python-app-dev$ curl http://localhost:3000/
```
...
```bash
python-app-dev$ sam logs -n HelloWorldFunction --stack-name python-app-dev --tail
```
...
```bash
python-app-dev$ pip install -r tests/requirements.txt --user
# unit test
python-app-dev$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
python-app-dev$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name python-app-dev
```
